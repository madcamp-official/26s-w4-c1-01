...
export const FLOORPLANS = [
  ...
  // 컷아웃 모서리는 벽·이웃 부속실에 '붙여' 둔다(2~5호는 판독값을 0.5m 이내에서 스냅했다).
  // 렌더가 부속실을 바닥에서 파내므로, 사이에 남은 20~50cm는 가구도 못 놓고 벽으로 둘러싸인
  // 못 쓰는 바닥 슬롯으로 찍힌다. 도면상 그 틈은 대부분 벽 두께·배관 샤프트다.
  {
  ...
  },
  {
    ...
    cutouts: [
      ...
      { x: 0, y: 2.8, w: 1.5, d: 1.6, kind: 'closet' },     // 보일러실(좌하단) — 주방 카운터와 맞닿음
  ...
  },
  {
    ...
    cutouts: [
      { x: 0, y: 0, w: 1.3, d: 1.0, kind: 'closet' },       // 보일러실 — 아래로 욕실과 맞닿음
      { x: 0, y: 1.0, w: 1.7, d: 1.8, kind: 'bath' },       // 욕실 — 좌측 외벽에 붙임
      { x: 0, y: 4.5, w: 1.5, d: 0.9, kind: 'entry' },      // 현관 — 좌측 외벽에 붙임
      { x: 3.3, y: 3.2, w: 1.2, d: 2.2, kind: 'kitchen' },  // 우측 세로 주방 — 우측·하단 외벽까지
    ],
    annex: { d: 0, bath: null, kitchen: { x: 0, w: 0 }, entry: { x: 0, w: 0 } },
  ...
  },
  {
    ...
    cutouts: [
      ...
      { x: 0.85, y: 2.85, w: 1.0, d: 3.1, kind: 'kitchen' },// 세로 주방 빌트인 — 현관과 맞닿음
      { x: 1.85, y: 4.45, w: 1.0, d: 1.5, kind: 'entry' },  // 현관
      { x: 2.85, y: 4.15, w: 2.55, d: 1.8, kind: 'bath' },  // 욕실 — 도면 '2,400'대로 우측 외벽까지
    ],
    annex: { d: 0, bath: null, kitchen: { x: 0, w: 0 }, entry: { x: 0, w: 0 } },
  ...
  },
  {
    ...
    cutouts: [
      { x: 0, y: 0, w: 2.0, d: 1.5, kind: 'entry' },        // 현관 — 욕실 벽까지(사이는 배관 샤프트)
      { x: 2.0, y: 0, w: 1.9, d: 1.85, kind: 'bath' },      // 욕실 — 아래로 보일러실과 맞닿음
