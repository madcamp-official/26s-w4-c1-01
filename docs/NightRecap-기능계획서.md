...
- **측정하는 신호는 손목 가속이 아니라 매트리스 전달 진동이다.** 손목 액티그래피는 사지의 직접 가속을 재지만, 이 앱은 폰이 잡는 뒤척임의 *간접* 진동을 쓴다 — 매트리스 강성·전달 특성에 좌우되고(딱딱한 매트리스에서 저하), 도달 범위 안(약 1~2m)의 동침자·반려동물 움직임이 함께 섞인다. 상용 앱(Sleep Cycle·Sleep as Android·Pillow) "폰만" 모드가 공통으로 요구하는 조건을 따른다: **폰은 협탁이 아니라 매트리스 위 몸 근처(어깨/가슴 높이)에 둔다** — 협탁 배치는 가속도 신호가 몸과 분리되어 사실상 무효다. 또한 **단독 취침을 전제**하며, 데모는 반드시 1인 취침 데이터로 한다(공유베드·도미토리에서는 옆사람 뒤척임을 착용자 wake로 오판).
- 30초 에폭별 움직임 카운트에 **Cole-Kripke/Sadeh형 가중 커널**을 적용해 상대적 sleep/wake를 추정한다. 단 이것은 **손목 활동 카운트용으로 피팅·검증된 계수를 매트리스 진동 카운트에 재보정 없이 적용하는 휴리스틱**이며, 손목 알고리즘의 직접 이식이 아니라 근사다(Kripke 2010: 계수·임계값은 기기·에폭 길이·착용 부위마다 재보정이 필요). 알고리즘은 "가중합 + 임계값"이라 Kotlin 수십 줄로 끝난다. 판정 로직은 **순수 함수**로 격리하고 단위 테스트로 검증한다(에폭 배열 입력 → sleep/wake 배열 출력, 골든 케이스 고정). 원본 가속도 스트림은 에폭 집계 후 폐기하고 `Epoch` 행만 남긴다.
- light/deep은 sleep 구간 내부의 움직임 밀도로 나눈 **휴리스틱**이며 별도 검증 근거가 없으므로 히프노그램은 접힌 섹션에 두고 배지를 단다.
- **정확도는 자체 대조 검증 전까지 미상으로 취급한다.** 78~80%(§10)는 손목 액티그래피 문헌치일 뿐 이 앱의 정확도가 아니다. 팀 실측 기간(D0~)에 검증된 스마트워치 또는 수면일지와 폰 파이프라인을 수 박 대조해 **자체 일치도**를 산출하고, 발표에서는 손목 수치 대신 이 자체 수치(또는 "정확도 미주장, 상대적 수면패턴 추정")를 제시한다.
...
**정직 3계명 (측정)**
1. sleep/wake·light/deep은 항상 **"추정치" 배지**와 함께 표시한다. 근거: 78~80%는 **손목 액티그래피 문헌 참고치**(npj Digital Medicine)이지 폰의 정확도가 아니다 — 폰 정확도는 자체 대조 검증 전까지 미상이며, 특히 wake(각성) 검출은 손목 기준보다 더 부실할 수 있다(미동이 매트리스로 전달 안 되면 수면으로 과대추정; 손목에서도 wake 특이도가 ~0.35~0.64로 낮아 78~80%는 대칭 정확도가 아님). 매트리스 진동이라는 다른 신호·다른 부착 위치임을 전제한다(§3-(a)-1).
...
| 항목 | 값 | 출처·근거 |
|---|---|---|
| 에폭 | 30초 | Cole-Kripke/Sadeh **커널을 차용한** 30초 판정 단위 — 손목 검증 알고리즘의 무보정 근사임을 명시 (스펙 NightRecap.md) |
...
| sleep/wake 판별 | **손목 문헌 참고치 78~80%** (폰 미검증) | 손목 액티그래피, npj Digital Medicine — **앱 정확도 아님**. wake 특이도 ~0.35~0.64로 비대칭(base-rate 편향, 수면 과대추정). 폰 자체 정확도는 실측 전 미상 → 발표에서 앱 성능으로 제시 금지 |
